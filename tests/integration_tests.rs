...
fn run_prompt_with_input(args: &[&str], input: &str) -> (i32, String, String) {
    let mut cmd = Command::new("./target/debug/askr")
        .args(args)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
fn test_help_output() {
    ...
    assert!(stdout.contains("Interactive CLI input tool"));
...
fn test_basic_required_validation() {
    ...
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--required", "Enter text:"], "hello world");

    assert_eq!(exit_code, 0);
    assert_eq!(stdout.trim(), "hello world");

    // Test empty input (should fail)
    let (exit_code, _stdout, stderr) = run_prompt_with_input(&["--required", "Enter text:"], "");
...
fn test_length_validation() {
    ...
    let (exit_code, _stdout, stderr) =
        run_prompt_with_input(&["--min-length", "5", "Enter text:"], "hi");
    ...
    let (exit_code, _stdout, stderr) =
        run_prompt_with_input(&["--max-length", "5", "Enter text:"], "hello world");
...
fn test_email_validation() {
    // Test valid email
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--validate-email", "Enter email:"], "user@example.com");
    ...
    // Test invalid email
    let (exit_code, _stdout, stderr) =
        run_prompt_with_input(&["--validate-email", "Enter email:"], "not-an-email");
...
fn test_number_validation() {
    ...
    let (exit_code, stdout, _stderr) = run_prompt_with_input(&["--integer", "Enter number:"], "42");
    ...
    let (exit_code, _stdout, stderr) =
        run_prompt_with_input(&["--integer", "Enter number:"], "not-a-number");

    assert_ne!(exit_code, 0);
    assert!(stderr.contains("number") || stderr.contains("Validation failed"));

    // Test float validation
    let (exit_code, stdout, _stderr) = run_prompt_with_input(&["--float", "Enter number:"], "3.14");
...
fn test_range_validation() {
    // Test valid range
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--range", "1-10", "Enter number:"], "5");
    ...
    // Test out of range
    let (exit_code, _stdout, stderr) =
        run_prompt_with_input(&["--range", "1-10", "Enter number:"], "15");
...
fn test_choice_validation() {
    // Test valid choice
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--choices", "red,green,blue", "Pick a color:"], "red");
    ...
    // Test invalid choice
    let (exit_code, _stdout, stderr) =
        run_prompt_with_input(&["--choices", "red,green,blue", "Pick a color:"], "yellow");
...
fn test_date_validation() {
    // Test valid date
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--date", "Enter date:"], "2025-01-15");
    ...
    // Test invalid date
    let (exit_code, _stdout, stderr) =
        run_prompt_with_input(&["--date", "Enter date:"], "not-a-date");
...
fn test_file_validation() {
    ...
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--file-exists", "Enter file:"], "Cargo.toml");
    ...
    let (exit_code, _stdout, stderr) =
        run_prompt_with_input(&["--file-exists", "Enter file:"], "nonexistent-file.txt");
...
fn test_json_output() {
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--output", "json", "--required", "Enter text:"], "hello");
...
#[test]
fn test_quiet_mode() {
    let (exit_code, stdout, _stderr) = run_prompt_with_input(&["--quiet", "--required"], "hello");
...
fn test_custom_prompt_text() {
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--required", "Please enter your name:"], "John Doe");
...
fn test_no_color_output() {
    let (exit_code, _stdout, _stderr) =
        run_prompt_with_input(&["--no-color", "--required", "Enter text:"], "hello");
...
fn test_raw_output_format() {
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--output", "raw", "--required", "Enter text:"], "hello");
...
fn test_unicode_input() {
    ...
    let (exit_code, stdout, _stderr) =
        run_prompt_with_input(&["--required", "Enter text:"], "مرحبا こんにちは");
...
fn test_shell_completion_generation() {
    ...
    assert_eq!(exit_code, 0);
    assert!(stdout.contains("#compdef askr"));
    ...
    assert_eq!(exit_code, 0);
    assert!(stdout.contains("__fish_askr"));
