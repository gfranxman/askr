use crossterm::{
    cursor::{MoveTo, MoveToColumn, MoveToNextLine, SavePosition, RestorePosition},
    terminal::{Clear, ClearType},
    ExecutableCommand,
};
use std::io::{self, Write};
use unicode_width::UnicodeWidthStr;
use super::colors::{Colorizer, ColoredText};
use crate::validation::{ValidationResult, Priority};
...
impl<W: Write + ExecutableCommand> Screen<W> {
    ...
    pub fn write_input(&mut self, input: &str, prompt_width: u16, error_pos: Option<usize>) -> io::Result<()> {
        ...
        if let Some(error_pos) = error_pos {
            ...
            if !valid_part.is_empty() {
                ...
                self.colorizer.write_colored(&mut self.writer, &valid_text)?;
            }
            
            if !invalid_part.is_empty() {
                let invalid_text = self.colorizer.invalid_text(invalid_part);
                self.colorizer.write_colored(&mut self.writer, &invalid_text)?;
            }
        } else {
            // All text is valid
    ...
    pub fn position_cursor_after_input(&mut self, input: &str, prompt_width: u16) -> io::Result<()> {
